...
import { Mountain, MapPin, Plane, Building2, Sparkles, Footprints, Hotel, Trees } from 'lucide-react';
...
export type MarkerType = 'airports' | 'cities' | 'highlights' | 'trekking' | 'accommodation' | 'nature' | 'mountain' | 'default';
...
const createCustomIcon = (type: string) => {
  ...
  const html = renderToStaticMarkup(
    <div className={`p-1.5 rounded-full shadow-md border-2 border-white ${bgColorClass} transition-transform hover:scale-110`}>
      {iconComponent}
    </div>
  );
...
const NepalMap = ({ markers = [], selectedMarkerId, onMarkerClick, onRegionClick }: NepalMapProps) => {
  ...
  return (
    <div className="h-full w-full rounded-xl overflow-hidden shadow-sm border border-white/10 z-0 relative bg-[#1a1a1a]">
      <MapContainer 
          ...
          >
            <Popup className="custom-popup">
              <div className="p-1 min-w-[150px]">
                <h3 className="font-bold text-sm text-gray-900">{marker.name}</h3>
